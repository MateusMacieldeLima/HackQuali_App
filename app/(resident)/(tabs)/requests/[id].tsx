...
export default function TicketDetailsScreen() {
  ...
  const fetchTicketDetails = async () => {
    try {
      ...
      // Buscar outros tickets do mesmo building (similar ao BuildingDetails)
      const { data: otherTickets, error: ticketsError } = await supabase
        .from('service_requests')
        .select('id, title, status, created_at')
        .eq('building_id', ticketData.building_id)
        .neq('id', id)
        .order('created_at', { ascending: false })
        .limit(10);

      if (ticketsError) {
        console.error('Error fetching building tickets:', ticketsError);
      }

      // Buscar histórico de manutenção do building (se existir a tabela)
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16 }}>
      ...
      {/* Informações do Empreendimento - Similar ao BuildingDetails */}
      <View
        style={{
